...
import { VirtualAnchor, virtualAnchorEncode } from "../ui/VirtualAnchor";
import {
    AifPipelineDiagram,
    AifRegionPlacementDiagram,
    AifTierDecisionDiagram,
} from "../diagrams/aif-internals-diagrams";
import {
    StringStorageDiagram,
    StringViewLifetimeDiagram,
} from "../diagrams/string-representation-diagrams";
...
const LinkedHeading: React.FC<LinkedHeadingProps> = ({
    ...
    const id = idProp || virtualAnchorEncode(props.children);
...
const Code: React.FC<React.HTMLAttributes<HTMLElement>> = ({ children, className, ...props }) => {
    ...
    return (
        <code
            className={cn(
                "px-1.5 py-0.5 rounded-md text-sm font-mono text-red-600 dark:bg-zinc-800 dark:text-red-400",
                className
            )}
            {...props}
        >
...
export const baseMDXComponents = {
    ...
    blockquote: (props: any) => (
        <blockquote
            className="my-5 rounded-r-lg border-l-4 border-violet-500 bg-violet-50/50 px-4 py-3 text-zinc-800 dark:border-violet-400 dark:bg-violet-950/20 dark:text-zinc-200"
            {...props}
        />
    ),
    ...
    thead: ({ className, ...props }: React.HTMLAttributes<HTMLTableSectionElement>) => (
        <thead
            className={cn(
                "bg-zinc-50 dark:bg-zinc-900/80 border-b border-zinc-200 dark:border-zinc-800",
                className
            )}
            {...props}
        />
...
export function createMDXComponents(customComponents?: Record<string, any>) {
    return {
        ...baseMDXComponents,
        ...customComponents,
