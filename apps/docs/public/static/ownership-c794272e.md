...
You can also end a value's life early with `drop(x)`, which frees it immediately. Because
`drop` **consumes** (moves) the value, the compiler guarantees it runs at most once — using a
value after dropping it, or dropping it twice, is a compile-time error rather than a
...
// let z = p.x     // ❌ Compile error: use of moved value: p
```

This makes manual, deterministic cleanup *safe* — you get C-level control over *when* memory
is released, with the compiler proving you can't get it wrong.
...
When you just need to *read* a value without taking ownership, **borrow** it. In Prismio,
borrowing is the *default* parameter convention — there is no `&` and no lifetime annotation.
See [Parameter Conventions](../functions/parameters.md).

```prismio
fn printLength(s: String) {       // a plain parameter is a shared borrow
...
To mutate the caller's value, mark the parameter `inout`; to take ownership, mark it `sink`.
...
```prismio
let data = getData()
process(data)       // moves data (process takes it by `sink`)
// log(data)        // ❌ use after move

// Solution 1: borrow instead — make process/log take a shared borrow (the default),
// so they don't consume `data`
process(data)
log(data)

// Solution 2: clone for an independent owned copy
process(data.clone())
log(data)
```
...
| Drop        | Cleanup when the owner goes out of scope — or explicitly via `drop(x)` |
| Move        | Transfer of ownership to a new variable or `sink` argument    |
| Copy        | Implicit duplication for cheap, stack-only types              |
| Clone       | Explicit deep copy for heap-allocated types                   |
| Borrow      | Access without taking ownership via [parameter conventions](../functions/parameters.md) (`let`/`inout`) — no `&`, no lifetimes |
