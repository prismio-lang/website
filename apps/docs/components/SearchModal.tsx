...
export default function SearchModal() {
    ...
    const results = useMemo(() => {
        ...
        return (docs as Docs[])
            .map((doc) => {
                ...
                else if (title.startsWith(needle)) score += 130;
                else if (title.includes(needle)) score += 100;
                if (tags.includes(needle)) score += 55;
                if (description.includes(needle)) score += 35;
                if (bodyLower.includes(needle)) score += 10;

                const index = bodyLower.indexOf(needle);
    ...
    return (
        <div className="fixed inset-0 z-[100] flex items-start justify-center bg-zinc-950/55 px-4 pt-[10vh] backdrop-blur-sm" onMouseDown={(event) => event.currentTarget === event.target && setIsOpen(false)}>
            <div role="dialog" aria-modal="true" aria-labelledby="search-title" className="w-full max-w-2xl overflow-hidden rounded-2xl border border-zinc-200 bg-white shadow-2xl dark:border-zinc-800 dark:bg-[#101014]">
