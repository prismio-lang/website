...
export default async function DocsPage({ params }: PageProps) {
    ...
    return (
        <div className="relative mx-auto flex w-full max-w-[100rem] gap-6 py-2 md:py-6">
            <article className="min-w-0 flex-1 px-0 md:px-6 lg:px-8">
                <script
                ...
                <nav aria-label="Page navigation" className="mt-10 grid gap-3 border-t border-zinc-200 pt-8 sm:grid-cols-2 dark:border-zinc-800">
                    {pager.previous ? (
                        <Link href={pager.previous.href} className="group rounded-xl border border-zinc-200 p-4 transition-colors hover:border-zinc-400 dark:border-zinc-800 dark:hover:border-zinc-600">
                            <span className="flex items-center gap-1 text-xs uppercase tracking-wide text-zinc-500"><ArrowLeft size={13} /> Previous</span>
                    ...
                    {pager.next && (
                        <Link href={pager.next.href} className="group rounded-xl border border-zinc-200 p-4 text-right transition-colors hover:border-zinc-400 dark:border-zinc-800 dark:hover:border-zinc-600">
                            <span className="flex items-center justify-end gap-1 text-xs uppercase tracking-wide text-zinc-500">Next <ArrowRight size={13} /></span>
            ...
            {headings.length > 0 && (
                    <aside className="sticky top-24 hidden h-[calc(100vh-7rem)] w-56 shrink-0 overflow-y-auto pl-4 2xl:block">
